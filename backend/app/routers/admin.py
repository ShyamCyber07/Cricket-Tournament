...
# --- Temporary Troubleshooting DB Diagnostics ---

@router.get("/admin/debug-db", status_code=status.HTTP_200_OK)
def debug_db(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin)
):
...
@router.get("/admin/run-migrate", status_code=status.HTTP_200_OK)
def run_migrate(
    current_user: User = Depends(require_admin)
):
    import io
