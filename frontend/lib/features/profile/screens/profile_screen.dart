          ...
          _buildGlassCard(
            child: Column(
              ...
              children: [
                ...
                _buildProfileDetailRow("Wins", "${tournament['matches_won'] ?? 0}", Icons.emoji_events_outlined),
                _buildProfileDetailRow("Career Runs", "${batting['runs'] ?? 0}", Icons.sports_cricket_outlined),
                _buildProfileDetailRow("Highest Score", "${batting['highest_score'] ?? 0}", Icons.trending_up),
                _buildProfileDetailRow("Batting Average", "${batting['average'] ?? 0.0}", Icons.analytics_outlined),
                _buildProfileDetailRow("Strike Rate", "${batting['strike_rate'] ?? 0.0}", Icons.speed_outlined),
                _buildProfileDetailRow("Career Wickets", "${bowling['wickets'] ?? 0}", Icons.bolt_outlined),
                _buildProfileDetailRow("Best Bowling", "${bowling['best_bowling'] ?? "0/0"}", Icons.track_changes),
                _buildProfileDetailRow("Economy Rate", "${bowling['economy'] ?? 0.0}", Icons.show_chart_outlined),
                _buildProfileDetailRow("Fifty Scores (50s)", "${batting['fifties'] ?? 0}", Icons.star_border_outlined),
                _buildProfileDetailRow("Century Scores (100s)", "${batting['hundreds'] ?? 0}", Icons.workspace_premium_outlined),
                _buildProfileDetailRow("Player of Match Awards", "${(_stats!['awards'] as List?)?.length ?? 0}", Icons.military_tech_outlined),
              ],
            ),
          ),
          const SizedBox(height: 24),
